...
this repository's commit history rather than in a design document checked in
here, so there is one place to read it and it cannot drift from the code.
...
the first turns a numeric cell that rendered as a timestamp into one that
renders 100x too large.
...
1. `XfRk.String` rendered *every* RK cell carrying a user-defined number format
   ...
   format actually was — so a cell carrying a numeric format rendered as a
   date. It now classifies the format string and takes the timestamp path only
   for a real date format. Date-formatted cells still render as RFC3339
   timestamps.
...
2. `RK.number` decoded the RK payload with an unsigned shift, so negative
   ...
   two decimals that fits 30 bits, which covers most two-decimal values.
   Upstream's master branch carries an equivalent fix.
...
failure mode is a visibly wrong cell rather than a silently wrong value, and
...
- it renders the date path through the format string (`goyymmdd`) rather than
  RFC3339, which breaks the ISO-timestamp shape callers read dates in;
