...
func TestRKNumber(t *testing.T) {
	tests := []struct {
		...
		// The hundredths flag was ignored on the integer variant, so a money
		// cell holding 9999.25 decoded as 999925.
...
// The money formats below must render as numbers and the date formats as
// timestamps. The timestamp output is not incidental: downstream consumers
// read .xls date cells as ISO timestamps, so preserving it is a requirement
// of this change rather than a leftover.
func TestXfRkString(t *testing.T) {
	const money = `#,##0.0000_);\(#,##0.0000\)`
	...
	tests := []struct {
		...
		{"user money int", 165, money, true, rkInt(9999011), "9999011"},
		{"user money int x100", 165, money, true, rkIntX100(999925), "9999.25"},
		{"user money int x100 negative", 165, money, true, rkIntX100(-999975), "-9999.75"},
		{"user money int negative", 165, money, true, rkInt(-9999011), "-9999011"},
		{"user money float", 165, money, true, rkFloat(0.5), "0.5"},
		{"user money float x100", 165, money, true, rkFloatX100(50), "0.5"},
		// User-defined date format: renders as a timestamp. All three
		// encodings are pinned, not just the plain integer: the timestamp
		// output is the contract downstream parsers read dates through, and
		// the fX100 and float branches are the two whose decode changed --
