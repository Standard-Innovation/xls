...
9999 placeholder space: an arbitrary-looking amount is very likely to collide
with a real cell in some statement, and a fixture value that reads as a
plausible royalty amount is indistinguishable from a leaked one on review.
...
the reader's behaviour differs across all of them:
  ...
  * RK "integer/100" -- the same, with the fX100 flag set. Excel and xlwt both
                        ...
                        decimals that fits 30 bits, which is most money.
...
MONEY = r'#,##0.0000_);\(#,##0.0000\)'  # user-defined: >= 164
...
CASES = [
    ('rk_int_money', 9999011, MONEY, 'RK integer'),
    ('rk_int_x100_money', 9999.25, MONEY, 'RK integer/100'),
    ('rk_int_x100_negative_money', -9999.75, MONEY, 'RK integer/100, negative'),
    ('number_money', 0.9999000000099991, MONEY, 'NUMBER: too many decimals for any RK form'),
